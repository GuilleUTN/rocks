...
def draw(img, players):
	...
	if(bluePlayers == []):
		...
		#img = cv.fromarray(img)
		cv2.putText(img, 'blue players not enough, need at least 1 blue player', (0,startPoint), font, 0.55, (0,0,255),1)
		img = np.asarray(img)
		startPoint += 25
		flag = False
	if(redPlayers == []):
		#img = cv.fromarray(img)
		#font=cv.InitFont(cv.CV_FONT_HERSHEY_COMPLEX, 0.5, 0.8, 0, 1, 5)
		cv2.putText(img, 'red players not enough, need at least 1 red player', (0,startPoint), font, 0.55, (0,0,255), 1)
		img = np.asarray(img)
		flag = False
